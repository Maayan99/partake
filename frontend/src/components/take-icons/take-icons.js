...
export default function TakeIcons({impactType, coins, duration, displayRewardIcon}) {
    const getDurationText = () => {
        if (duration < 60) {
            return <p className="text-[7pt] -translate-y-[153%] leading-3 text-center drop-shadow-md">{duration}<br/>Min</p>;
        } else if (duration < 1440) {
            if (duration === 60) {
                return <p className="text-[7pt] -translate-y-[153%] leading-3 text-center drop-shadow-md">1<br/>Hour</p>
            }
            return <p className="text-[7pt] -translate-y-[153%] leading-3 text-center drop-shadow-md">{duration / 60}<br/>Hours</p>;
        } else if (duration < 10080) {
            if (duration === 1440) {
                return <p className="text-[7pt] -translate-y-[153%] leading-3 text-center drop-shadow-md">1<br/>Days</p>;
            }
            return <p className="text-[7pt] -translate-y-[153%] leading-3 text-center drop-shadow-md">{duration / 1440}<br/>Days</p>;
        } else {
            if (duration === 10080) {
                return <p className="text-[7pt] -translate-y-[153%] leading-3 text-center drop-shadow-md">1<br/>Week</p>;
            }
            return <p className="text-[7pt] -translate-y-[153%] leading-3 text-center drop-shadow-md">{duration / 10080}<br/>Weeks</p>;
        }
    }
        ...
        <div className="mt-3.5 ml-2.5 flex gap-2">
            ...
            {duration &&
                <div className="text-white w-12 h-12">
                    <svg className="w-12 h-12 fill-white drop-shadow-md" viewBox="0 0 64.57 65.694">
